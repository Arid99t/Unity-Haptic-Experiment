...
public class New_Exp_Manager : MonoBehaviour
{
    [Header("UI Elements")]
    ...
    [Header("Components")]
    [SerializeField] private New_BoxDeformer boxDeformer;
    ...
    private readonly float[] targetCompressions = { 0.05513958f, 0.05132383f, 0.04690298f, 0.04121545f, 0.03666281f, 0.0331879f, 0.03039168f, 0.02516461f, 0.0208567f, 0.01716085f };
    ...
    private void InitializeCsvFile()
    {
        ...
        string header = "Step,Pressure,Time,StepTime,Distance,Accuracy,CubePointX,CubePointY,CubePointZ,CubeX,CubeY,CubeZ,MaterialConstant,TargetDistance,ScaleX,TargetCompression,CompressionError,Timer_Non_Visual,MaxDeformation";
    ...
    private void StartNewStep()
    {
        ...
        // Generate a random MaxDeformation between 0.1f and 0.9f
        float newMaxDeformation = UnityEngine.Random.Range(0.1f, 0.9f);
        boxDeformer.SetMaxDeformation(newMaxDeformation);

        // Optionally, you can log the new MaxDeformation value
        Debug.Log($"Step {currentStep}: MaxDeformation set to {newMaxDeformation}");

        // Change material constant every 10 steps (if needed)
    ...
    private void CollectData()
    {
        ...
        float maxDeformation = boxDeformer.GetMaxDeformation(); // Ensure your BoxDeformer has this method
        ...
        string dataLine = $"{currentStep},{pressure},{time},{stepTime:F4},{distance:F4},{accuracy:F2}," +
                          ...
                          $"{materialConstant},{targetDistance:F4},{scaleX},{targetCompression:F5},{compressionError:F5},{timerNonVisual:F4},{maxDeformation:F4}";
